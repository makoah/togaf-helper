...
export default function StakeholdersPage() {
  ...
  const getInfluenceColor = (level: string) => {
    switch (level) {
      case 'high': return 'bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/30';
      case 'medium': return 'bg-[#00BCD4]/20 text-[#00BCD4] border border-[#00BCD4]/30';
  ...
  const getInterestColor = (level: string) => {
    switch (level) {
      case 'high': return 'bg-[#007BFF]/20 text-[#007BFF] border border-[#007BFF]/30';
      case 'medium': return 'bg-[#00BCD4]/20 text-[#00BCD4] border border-[#00BCD4]/30';
      case 'low': return 'bg-gray-700/50 text-gray-400 border border-gray-600';
      default: return 'bg-gray-700/50 text-gray-400 border border-gray-600';
    }
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-start mb-8">
        ...
        <button
          ...
          className="px-6 py-3 bg-[#00BCD4] text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-[#00BCD4]/30 transition-all hover:-translate-y-0.5"
        >
          + Add Stakeholder
        </button>
      </div>
      ...
      <div className="rounded-2xl bg-[#0F172A] border border-gray-800 p-6 mb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl bg-[#D4AF37]/10 border border-[#D4AF37]/30 p-4">
            <h3 className="text-sm font-medium text-[#D4AF37] mb-2">High Influence / High Interest</h3>
            <p className="text-xs text-gray-500 mb-3">Key Players - Manage Closely</p>
            <div className="space-y-2">
              {stakeholders
                .filter(s => s.influence === 'high' && s.interest === 'high')
                .map(s => (
                  <div key={s.id} className="text-sm bg-[#D4AF37]/20 text-[#D4AF37] px-3 py-1.5 rounded-lg border border-[#D4AF37]/30">
                    {s.name}
                  </div>
                ))}
            </div>
          </div>
          <div className="rounded-xl bg-[#00BCD4]/10 border border-[#00BCD4]/30 p-4">
            <h3 className="text-sm font-medium text-[#00BCD4] mb-2">High Influence / Low Interest</h3>
            <p className="text-xs text-gray-500 mb-3">Keep Satisfied</p>
            <div className="space-y-2">
              {stakeholders
                .filter(s => s.influence === 'high' && s.interest === 'low')
                .map(s => (
                  <div key={s.id} className="text-sm bg-[#00BCD4]/20 text-[#00BCD4] px-3 py-1.5 rounded-lg border border-[#00BCD4]/30">
                    {s.name}
                  </div>
                ))}
            </div>
          </div>
          <div className="rounded-xl bg-[#007BFF]/10 border border-[#007BFF]/30 p-4">
            <h3 className="text-sm font-medium text-[#007BFF] mb-2">Low Influence / High Interest</h3>
            <p className="text-xs text-gray-500 mb-3">Keep Informed</p>
            <div className="space-y-2">
              {stakeholders
                .filter(s => s.influence === 'low' && s.interest === 'high')
                .map(s => (
                  <div key={s.id} className="text-sm bg-[#007BFF]/20 text-[#007BFF] px-3 py-1.5 rounded-lg border border-[#007BFF]/30">
                    {s.name}
                  </div>
                ))}
            </div>
          </div>
          <div className="rounded-xl bg-[#1A2B48]/50 border border-gray-700 p-4">
            ...
            <div className="space-y-2">
              {stakeholders
                ...
                .map(s => (
                  <div key={s.id} className="text-sm bg-[#1A2B48] text-gray-300 px-3 py-1.5 rounded-lg border border-gray-700">
                    {s.name}
                  </div>
                ))}
            </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-[#0F172A] border border-gray-700 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#1A2B48] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-[#00BCD4] focus:border-transparent transition-all"
                  required
                />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-[#1A2B48] border border-gray-700 rounded-xl text-white focus:ring-2 focus:ring-[#00BCD4] focus:border-transparent transition-all"
                  >
                    <option value="high" className="bg-[#1A2B48]">High</option>
                    <option value="medium" className="bg-[#1A2B48]">Medium</option>
                    <option value="low" className="bg-[#1A2B48]">Low</option>
                  </select>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-[#1A2B48] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-[#00BCD4] focus:border-transparent transition-all"
                    onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addConcern())}
                  />
                  <button
                    type="button"
                    onClick={addConcern}
                    className="px-4 py-3 bg-[#1A2B48] text-gray-300 rounded-xl hover:bg-[#1A2B48]/80 transition-colors border border-gray-700"
                  >
                    Add
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {formData.concerns.map((concern, i) => (
                    <span key={i} className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#00BCD4]/20 text-[#00BCD4] rounded-lg text-sm border border-[#00BCD4]/30">
                      {concern}
                      <button
                        type="button"
                        onClick={() => removeConcern(i)}
                        className="text-[#00BCD4] hover:text-red-400 transition-colors"
                      >
                        ×
                      </button>
                    </span>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-[#1A2B48] border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-[#00BCD4] focus:border-transparent transition-all resize-none"
                />
              </div>
              <div className="flex justify-end gap-3 pt-4">
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-6 py-3 border border-gray-700 text-gray-300 rounded-xl hover:bg-[#1A2B48] transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-6 py-3 bg-[#00BCD4] text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-[#00BCD4]/30 transition-all"
                >
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-800">
              {stakeholders.map((stakeholder) => (
                <tr key={stakeholder.id} className="hover:bg-[#1A2B48]/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {stakeholder.concerns.slice(0, 2).map((c, i) => (
                        <span key={i} className="text-xs px-2 py-1 bg-[#1A2B48] text-gray-300 rounded-lg border border-gray-700">
                          {c}
                        </span>
                      ))}
                  ...
                  <td className="px-6 py-4 text-center">
                    <button
                      ...
                      className="text-[#007BFF] hover:text-[#007BFF]/80 mr-4 transition-colors"
                    >
                      Edit
                    </button>
                    <button
